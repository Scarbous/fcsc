<?php
namespace Scarbous\lauer_template\ViewHelpers;
...
class FalViewHelper extends \TYPO3\CMS\Fluid\Core\ViewHelper\AbstractViewHelper {
    ...
    public function render($data, $as = "items", $table = "tt_content", $field = "image") {
		...
		foreach($items as $key => $item) :
			$resultItems[$key] = $item->getProperties();
			$resultItems[$key]['publicUrl'] = $item->getPublicUrl();
			$resultItems[$key]['mimeTypeExtension'] = $this->matchMimeTyoe($resultItems[$key]['mime_type']);
		endforeach;

        $this->templateVariableContainer->add($as, $resultItems);
	...
	private function matchMimeTyoe($mimetype) {
		$mimeTypeArray = array(
...
'x-world/x-vrml' => 'xof'
	...
	return $mimeTypeArray[$mimetype] ? $mimeTypeArray[$mimetype] : false;
